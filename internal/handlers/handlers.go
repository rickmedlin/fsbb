...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	if !form.Valid() {
		...
		http.Error(w, "form data is not valid", http.StatusSeeOther)
		render.Template(w, r, "make-reservation.page.tmpl", &models.TemplateData{
			Form: form,
			Data: data,
		})
	...
	htmlMessage = fmt.Sprintf(`
		<strong>Reservation Confirmation</strong><br><br>
		A reservation was made by %s %s for %s from %s to %s.
		`, reservation.FirstName,
...
// AdminDashboard allows users with access to review site data.
func (m *Repository) AdminDashboard(w http.ResponseWriter, r *http.Request) {
	render.Template(w, r, "admin-dashboard.page.tmpl", &models.TemplateData{})
}

// AdminAllReservations displays all reservations in admin dashboard.
...
// AdminNewReservations displays new reservations in admin dashboard.
...
func (m *Repository) AdminReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	// assume no month or year is specified.
	now := time.Now()
	...
	for _, x := range rooms {
		...
		for _, y := range restrictions {
			if y.ReservationID > 0 {
				// If reservation ID > 0, it is a reservation.
				for d := y.StartDate; !d.After(y.EndDate); d = d.AddDate(0, 0, 1) {
					reservationMap[d.Format("2006-01-2")] = y.ReservationID
				}
			} else {
				// Otherwise, it's a reservation block.
